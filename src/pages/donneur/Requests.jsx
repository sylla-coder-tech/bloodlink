  ...
  return (
    <div>
      ...
      <div style={{
        ...
        ℹ️ <strong>Comment ça fonctionne :</strong> Le CNTS vous convoque tous les 3–4 mois pour renouveler le stock sanguin, ou en cas d'urgence. Vous n'avez pas accès aux demandes des structures — c'est le CNTS qui coordonne tout.
      </div>
            ...
            <Card key={conv.id} style={{ padding: '16px', borderLeft: `4px solid ${statutColor[conv.statut] || 'var(--orange)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '8px' }}>
                <div>
                  ...
                  {conv.date_rdv && (
                    <p style={{ margin: '0', color: 'var(--text3)', fontSize: '12px' }}>
                      📅 Rendez-vous : {new Date(conv.date_rdv).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                    </p>
                  )}
                ...
                <span style={{
                  ...
                  {statutLabel[conv.statut] || conv.statut}
                </span>
              </div>
              ...
              {conv.statut === 'en_attente' && (
                <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
                  <Button
                    ...
                    onClick={() => handleRepondre(conv.id, 'confirmée')}
                    disabled={respondingId === conv.id}
                  >
                    {respondingId === conv.id ? 'Envoi...' : '✅ Confirmer'}
                  </Button>
                  <Button
                    variant="ghost" size="sm"
                    onClick={() => handleRepondre(conv.id, 'refusée')}
                    disabled={respondingId === conv.id}
                  >
